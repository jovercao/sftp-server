...
exports = module.exports = function(config, log, glob, User, File, system) {
    ...
    return () => {
        ...
        app.param('username', (req, res, next, id) => {
            // TODO: 添加用户路径传入User对象
            try {
                const user = config.get('sftp:users').find(user => user.username === this.username);
                if (!user) {
                    const err = new Error(`User does not exist: ${id}`);
                    err.code = 'USER404';
                    return next(err);
                }
                req.user = new User(id, user.rootDir);
                return next();
            } catch (err) {
                next(err);
            }
        ...
        app.param('userFile', (req, res, next, id) => {
            ...
            req.userFile = new File(req.user.username, req.user.rootDir, id);
        ...
        app.route('/api/users')
            .get((req, res, next) => {
                return Object.keys(config.get('sftp:users'));
                // return glob('*/', {
