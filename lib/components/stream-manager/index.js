...
exports = module.exports = function(fs, config, log) {
    ...
    class StreamManager {
        ...
        getLocalPath(reqPath) {
            return path.join(this.client.user.rootDir, reqPath);
        }

        getRelativeUserPath(p) {
            return path.relative(this.client.user.rootDir, p);
        }
        ...
        onREALPATH(reqID, reqPath) {
            let retPath = path.resolve('/', reqPath);
            if (os.platform() === 'win32') {
                // windows平台中的代码盘符长度
                const win32PatterLen = process.cwd().split(path.sep)[0].length;
                retPath = retPath.substr(win32PatterLen).replace(/\\/g, '/');
            }
            const localPath = this.getLocalPath(retPath);
            ...
            return fs.statAsync(localPath)
                .catch(error => {
                    return this.sendStatus(reqID, STATUS_CODE.NO_SUCH_FILE);
                })
                .then((stats) => {
                    return [
                        {
